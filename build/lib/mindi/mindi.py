...
def nonbdna_register(mode):
    def nonbdna(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if 'maxspacer' in kwargs:
                maxspacer = kwargs['maxspacer']
            elif len(args) > 2:
                maxspacer = args[2]
            else:
                # fallback to default
                maxspacer = int(Defaults.maxspacer)
            ...
            match mode:
                ...
                case 'DR':
                    command = f"{self.nonBDNA} -seq {accession} -out {rand_accession_name} -minDRrep {minrep} -maxDRspacer {maxspacer} -maxMRspacer -skipAPR -skipSTR -skipMR -skipIR -skipGQ -skipZ -skipSlipped -skipCruciform -skipTriplex -skipWGET"
                case 'STR':
        ...
        for seqID, seq in parse_fasta(accession):
            ...
            for _, row in temp.iterrows():
                ...
                if self.cur_mode == "MR":
                    ...
                    assert arm_seq + spacer_seq + reverse(arm_seq) == fasta_seq
