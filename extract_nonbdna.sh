...
then
  ...
  if [[ $mode == "g4" ]]; then
    ...
    snakemake --snakefile nonbdna_pipe.py --configfile config/config_IR.server.yaml -j $j --latency-wait 45 --cluster-config config/cluster_overlap.yaml --cluster "sbatch -p {cluster.partition} -t  {cluster.time} --mem={cluster.mem} -c {cluster.ncpus} --nodes={cluster.nodes} -o jobOut/{cluster.jobName}-%j.out -J {cluster.jobName} -e jobOut/{cluster.jobName}-%j.err"
  elif [[ $mode == "MR" ]]; then
    snakemake --snakefile nonbdna_pipe.py --configfile config/config_MR.server.yaml --latency-wait 5 --cluster-config config/cluster_overlap.yaml --cores $j
  else
