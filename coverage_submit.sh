...
then
	...
	snakemake --snakefile bedsnake_coverage.py --configfile config_coverage/config_coverage.MR.server.yaml -j $j --latency-wait 5 --cluster-config config/cluster_overlap.yaml --cluster "sbatch -p {cluster.partition} --account={cluster.account} -t  {cluster.time} --mem={cluster.mem} -c {cluster.ncpus} --nodes={cluster.nodes} -o jobOutCoverage${mode}/{cluster.jobName}-%j.out -J {cluster.jobName} -e jobOutCoverage${mode}/{cluster.jobName}-%j.err"
    else
	echo "Request could not be fullfiled. Reason: Invalid mode $mode."
	exit 1
    fi
